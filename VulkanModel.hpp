...
namespace vks
{
	...
	struct Model {
		...
		//static const int defaultFlags = aiProcess_FlipWindingOrder |aiProcess_Triangulate | aiProcess_JoinIdenticalVertices | aiProcess_CalcTangentSpace | aiProcess_GenSmoothNormals;
		...
		bool loadFromFile(const std::string& filename, vks::VertexLayout layout, vks::ModelCreateInfo *createInfo, vks::VulkanDevice *device, VkQueue copyQueue, const int flags = defaultFlags)
		{
			...
			if (pScene)
			{
				...
				for (unsigned int i = 0; i < pScene->mNumMeshes; i++)
				{
					...
					parts[i].matIndex = paiMesh->mMaterialIndex;
