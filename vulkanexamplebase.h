...
public:
    ...
    float xAngle = 0.5f, zAngle = 0.f;
    glm::vec3 vEyeTarget = glm::vec3 (3.5f, 2.8f, 0.f);
    //glm::vec3 vEyeTarget = glm::vec3 (0., 0.f, 0.f);
    //glm::vec3 vLookInit = glm::normalize (glm::vec3 (0.0f, -0.7f, 0.7f));
    glm::vec3 vLook = glm::normalize (glm::vec3(0.f,0.7f,0.4f));  // Camera vLook Vector
    float eyeDist = 12.0f;
    glm::vec3 vEyePos =  glm::vec3();
...
//
    ...
    */
    ...
    virtual void buttonDown(uint8_t);
    virtual void buttonUp(uint8_t);
...
int main(const int argc, const char *argv[])													    \
{																									\
    for (size_t i = 0; i < argc; i++) { VulkanExemple::args.push_back(argv[i]); };  				\
    vulkanExample = new VulkanExemple();															\
    vulkanExample->initVulkan();																	\
    vulkanExample->setupWindow();					 												\
    vulkanExample->initSwapchain();																	\
    vulkanExample->prepare();																		\
