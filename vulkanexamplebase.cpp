...
//    ss << std::fixed << std::setprecision(3) << "(" << boardPos.x << ", " << boardPos.y << ")";
//    textOverlay->addText(ss.str(), 5.0f, 25.0f, VulkanTextOverlay::alignLeft);
...
void VulkanExampleBase::handleEvent(const xcb_generic_event_t *event)
{
    ...
    case XCB_MOTION_NOTIFY:
    {
        ...
        float diffX = mousePos.x - (float)motion->event_x;
        float diffY = mousePos.y - (float)motion->event_y;
        if (diffX==0&&diffY==0)
            break;
        ...
        if (mouseButtons.left)
		{
            zAngle += diffX * rotationSpeed;
            xAngle -= diffY * rotationSpeed;
			viewUpdated = true;
		}
		if (mouseButtons.right)
		{
		...
		if (mouseButtons.middle)
		{
            vEyeTarget += glm::vec3(diffX*0.01f,-diffY*0.01f,.0f);
            viewUpdated = true;
		}
...
void VulkanExampleBase::buttonDown(uint8_t) {}
void VulkanExampleBase::buttonUp(uint8_t) {}
void VulkanExampleBase::mouseMove(float, float) {}
